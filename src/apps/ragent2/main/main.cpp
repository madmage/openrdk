...
vector<string> loadQuotes() {
	ifstream aphstream((string() + OpenRDK_ROOT + "/docs/aphorisms.txt").c_str());
...
void editing()
{
	...
	do {
		...
		if (c == 'a') {
			...
			string scmd = string() + "ls " + OpenRDK_OUTPUT_TREE;
			scmd += "/lib*/*module.* | sed -e 's/.*lib[0-9a-zA-Z]*\\/lib\\(.*\\)\\..*/\\1/g' | sort";
			int dummy = system(scmd.c_str()); dummy = dummy;
			cout << "Type the name of the library of the module to add (e.g. 'navigatormodule'): ";
			string libraryName;
			cin >> libraryName;
