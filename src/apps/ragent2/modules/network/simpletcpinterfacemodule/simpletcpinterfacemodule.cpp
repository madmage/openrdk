	...
	void SimpleTcpInterfaceModule::processCmd(cstr cmd, TCPSocket* tcpSocket)
	{
		...
		try {
			...
			else if (cmdline[0] == "getVectorRItemOnMap")
			{
				...
				{
					RDK2::RMaps::RPoint2dOnMap* point2d = dynamic_cast < RDK2::RMaps::RPoint2dOnMap*>(*it);
					if(point2d)
					{
						os << "RPoint2dOnMap$ " << point2d->x << " " << point2d->y << " " << RDK2::RGraphics::getColorName(point2d->color)
							<< " " << point2d->drawWidth << " ; ";
						continue; 
					}

					RDK2::RMaps::REllipseOnMap* ellipse = dynamic_cast < RDK2::RMaps::REllipseOnMap*>(*it);
					if(ellipse)
					{
						os << "REllipseOnMap$ " << ellipse->x << " " << ellipse->y << " " << ellipse->theta << " " << ellipse->a << " " << ellipse->b 
							<< " " << ellipse->filled << " " << RDK2::RGraphics::getColorName(ellipse->color) << " " << ellipse->drawWidth << " ; ";
						continue;
					}

					RDK2::RMaps::RLine2dOnMap* line2d = dynamic_cast < RDK2::RMaps::RLine2dOnMap*>(*it);
					if(line2d)
					{
						os << "RLine2dOnMap$ " << line2d->p0.x << " " << line2d->p0.y << " " << line2d->p1.x << " " << line2d->p1.y << " " 
