...
void RqGlViewer::addPose(const string& url)
{
	...
	shownPoses.insert(make_pair(v[0], sp));
}

void RqGlViewer::clearPoses()
{
	bool exiting = false; QT_THREAD_GUARD()
	shownPoses.clear();
}
...
void RqGlViewer::drawPoses()
{
	...
	for (map<string, ShownPose>::iterator it = shownPoses.begin(); it != shownPoses.end(); ++it) {
		...
		RPoint2od pose = guiSession->getPose(it->first);
				...
				if (ellipse->filled)
				{
					...
					for(float t = 0; t <= TWOPI; t += dth)
					{
						...
						glVertex2f(p.x,p.y);
								...
								//p.x,
								//ellipse->a * cos(t) + ellipse->x,
								//ellipse->b * sin(t) + ellipse->y
								//);
					glEnd();
				}
				else
