...
{
public:
	CountDown(double s=0):sec(s) { start(); }
	inline void start(double s) { sec=s; t.start(); }
	inline void start() { t.start(); }
	inline bool end() { return getSeconds() < 0; }
	inline double getMs() { return getSeconds() * 1000; }
	inline double getSeconds() { Timestamp now; return sec; }// - (now.getSeconds() - t.getSeconds()); }
